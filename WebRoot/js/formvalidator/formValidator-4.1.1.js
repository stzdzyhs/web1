...
$.formValidator = 
{
	...
	ajaxForm : function(validatorGroup,option,formid)
	{
		...
		$("#"+formid).find("input[type='submit']").focus();
		if(!$.formValidator.pageIsValid(validatorGroup)){return false};
		var ls_url = setting.url;
		var parm = $('#'+formid).formSerialize();
		if (setting.type.toUpperCase() == 'GET') {
	        ls_url += (ls_url.indexOf('?') >= 0 ? '&' : '?') + parm;
	        setting.data = null;  // data is null for 'get'
	    }
	    else
	        setting.data = parm; // data is the query string for 'post'
	...
	ajaxValid : function(returnObj)
	{
		...
		if (setting.type.toUpperCase() == 'GET') {
	        setting.data = null;  // data is null for 'get'
	        ls_url = ls_url + "clientid=" + id + "&" +(setting.randNumberName ? setting.randNumberName+"="+((new Date().getTime())+Math.round(Math.random() * 10000)) : "");
		}
	    else
	        setting.data = parm; // data is the query string for 'post'
