...
function resetOperBtn(checkboxName){
	...
	if(sets.length<=0){
		...
		var contentType=null,positionType=null,origin=null,contentType1=null,positionType1=null;
		contentType1 = $("#contentType_"+sets[0].value).val();
		positionType1 = $("#positionType_"+sets[0].value).val();
	   var i=0;
	   sets.each(function(){
	   		contentType = $("#contentType_"+sets[i].value).val();
			positionType = $("#positionType_"+sets[i].value).val();
			origin = $("#origin_"+sets[i].value).val();
			if(i>0){
				if(contentType!=contentType1){
					originBtnDis = true;
				}
				if(contentType==positionType1){
					originBtnDis = true;
				}
			}
